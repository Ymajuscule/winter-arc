...
- [~] **Auth flow** — Supabase Auth, magic link + Apple/Google. Screen UI built (`apps/mobile/src/app/auth.tsx`, 2026-08-28, matches CLAUDE.md §3's spec). `services/supabase.ts`'s client now has real credentials (`apps/mobile/.env`, gitignored — see SESSION-LOG.md for the values) but the screen still doesn't call `signInWithOtp` — that wiring is a small follow-up now that the client is live. Apple/Google buttons not added (need their own OAuth setup).
