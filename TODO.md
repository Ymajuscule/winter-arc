...
- [~] **Auth flow** — Supabase Auth, magic link + Apple/Google. Screen UI built (`apps/mobile/src/app/auth.tsx`, 2026-08-28, matches CLAUDE.md §3's spec), but not wired to a real `signInWithOtp` call — `services/supabase.ts`'s client is `null` until a project is linked. Apple/Google buttons not added (need their own OAuth setup once linked).
...
- [ ] Arc creation & lifecycle (default 90 days, difficulty tiers) — the Recap screen shows a computed 90-day Arc, but nothing persists an `arcs` row anywhere (no backend linked); Arc lifecycle (pause/complete/vacation mode) is entirely unbuilt
- [ ] Habits — 5 types, completion → `award-habit-xp` call. Only `boolean`-shaped completion exists in the UI so far (tap-to-complete on the dashboard); numeric/duration/counter/distance input UIs aren't built, and completion calls `app-store`'s local `completeHabit` (optimistic game-engine math, see its file header), not the real Edge Function — no Supabase project linked yet
- [x] Dashboard — header, today hero, habit list *(2026-08-28)*. Daily quests/weekly progress/boss card intentionally NOT built as fake UI — `rotate-quests` doesn't exist so there's no real quest to show; building the card first would mean faking data
